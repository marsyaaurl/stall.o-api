...
import {
  createSellerReservation,
  getSellerReservations,
  cancelSellerReservation,
  stockSellerReservation,
...
router.patch("/:reservationId/cancel", authenticateJWT, authorizeRoles(UserRole.SELLER), async (req, res) => {
  try {
    const result = await cancelSellerReservation(req.user!.id, req.params.reservationId as string);
...
router.patch("/:reservationId/stock", authenticateJWT, authorizeRoles(UserRole.SELLER), async (req, res) => {
  try {
    const result = await stockSellerReservation(req.user!.id, req.params.reservationId as string);
