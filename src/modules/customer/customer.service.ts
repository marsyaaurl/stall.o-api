...
import { GetProductsQuerySchema, CreateOrderSchema } from "../customer/customer.schema.js";
...
export async function getMachineProducts(
    machineId: string,
    filters: GetProductsQuerySchema
) {
    ...
    const productsMap: { [productId: string]: any } = {};
...
export async function getProductDetail(machineId: string, productId: string) {
    const batches = await prisma.productBatch.findMany({
        where: {
            productId,
            status: "ACTIVE",
...
export async function createCustomerOrder(data: CreateOrderSchema) {
...
export async function claimPickupCode(pickupCode: string) {
    return await prisma.$transaction(async (tx) => {
