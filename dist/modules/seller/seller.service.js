...
export async function getSellerDashboardData(sellerId) {
    ...
    const [activeProductsCount, productsInMachinesAgg, todaySalesAgg, expiringSoonCount, expiringBatches, startingReservations, activeReservations, inventory, recentSalesItems,] = await Promise.all([
    ...
    expiringBatches.forEach((batch) => {
        const hoursLeft = Math.max(0, Math.round((batch.expiresAt.getTime() - now.getTime()) / (1000 * 60 * 60)));
        actionRequired.push({
    ...
    startingReservations.forEach((res) => {
        const minsLeft = Math.max(0, Math.round((res.startTime.getTime() - now.getTime()) / (1000 * 60)));
        actionRequired.push({
